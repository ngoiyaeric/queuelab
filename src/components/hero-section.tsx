...
export function HeroSection() {
    ...
    return (
        ...
        >
            ...
            <motion.div
                ref={sphereRef}
                className="absolute size-64 md:size-96 bg-blue-500 rounded-full border border-white/20 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 shadow-[-20px_-20px_50px_rgb(255,255,255,0.5),-20px_-20px_80px_rgb(255,255,255,0.1),0_0_50px_rgb(0,0,255)] z-10"
                style={{ background: sphereBackground }}
            />
            
            {/* First Ring with Mini Planets */}
            <motion.div
                style={{ translateY: "-50%", translateX: "-50%" }}
                animate={{ rotate: "1turn" }}
                transition={{ duration: 60, repeat: Infinity, ease: "linear" }}
                className="absolute size-[344px] md:size-[580px] border border-white opacity-20 rounded-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10"
            >
            ...
            <motion.div
                ...
                animate={{ rotate: "-1turn" }}
                transition={{ duration: 60, repeat: Infinity, ease: "linear" }}
                className="absolute size-[444px] md:size-[780px] rounded-full border border-white/20 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 border-dashed z-10"
            />
            
            {/* Third Ring with Mini Planets */}
            <motion.div
                style={{ translateY: "-50%", translateX: "-50%" }}
                animate={{ rotate: "1turn" }}
                transition={{ duration: 90, repeat: Infinity, ease: "linear" }}
                className="absolute size-[544px] md:size-[980px] rounded-full border border-white opacity-20 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10"
            >
                <div className="absolute size-2 bg-white rounded-full top-1/2 left-0 -translate-x-1/2 -translate-y-1/2" />
            ...
            {/* Text Content */}
            <div className="container relative mt-20 md:mt-32 z-0">
                <h1 className="text-8xl md:text-[168px] md:leading-none font-semibold bg-white tracking-tighter bg-clip-text text-transparent text-center bg-[radial-gradient(100%_100%_at_top_left,rgba(255,255,255,0.8),rgba(255,255,255,0.7),rgba(0,0,255,0.3))] [text-shadow:2px_2px_4px_rgba(0,0,0,0.4),-1px_-1px_2px_rgba(255,255,255,0.3),0_0_10px_rgba(0,0,255,0.5)]">
                    QCX
                </h1>
                <p className="font-handwriting text-lg md:text-xl max-w-xl mx-auto text-white/70 mt-5 text-center">
                    is a multi-agent intelligence platform for exploration and automation. Your environment-aware planetary copilot for your
                </p>
