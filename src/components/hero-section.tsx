...
export function HeroSection() {
    ...
    const sphereBackground = useTransform(
        ...
        ([latestGradientX, latestGradientY]) => {
            if (theme === 'light') {
                return `radial-gradient(50% 50% at ${latestGradientX}% ${latestGradientY}%, hsl(45, 33%, 94%), hsl(120, 40%, 30%) 37.7%, hsl(48, 33%, 98%))`
            }
            return `radial-gradient(50% 50% at ${latestGradientX}% ${latestGradientY}%, white, rgb(0,0,255) 37.7%, rgb(24,0,66))`
        }
    );
            ...
            <motion.section
                ...
                <div className="container relative flex flex-col items-center justify-center h-full">
                    ...
                    <motion.div
                        ...
                        <p className={`text-center text-lg md:text-xl ${theme === 'light' ? 'text-natureGreenLight' : 'text-white/70'} tracking-tight px-4 mt-5`}>
                            Our mission is to democratize and interoperate to environment forecasts and digital replenishing into our explorative and autonomous systems.
                        </p>
