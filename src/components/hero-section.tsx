...
export function HeroSection() {
    ...
    return (
        ...
        >
            ...
            {/* Hero Section Content - Arranged with QCX above description */}
            <div className={"container relative text-center mt-10 z-20"}>
                <motion.h1
                    className={"text-4xl md:text-6xl font-bold text-white tracking-tighter bg-clip-text text-transparent bg-[radial-gradient(100%_100%_at_top_left,rgba(255,255,255,0.8),rgba(255,255,255,0.7),rgba(0,0,255,0.3))] [text-shadow:2px_2px_4px_rgba(0,0,0,0.4),_-1px_-1px_2px_rgba(255,255,255,0.3),_0_0_10px_rgba(0,0,255,0.5)] mb-4"}
                    style={{ display: !isAnimationVisible ? 'block' : 'none' }}
                >
                    QCX
                </motion.h1>
                <p className={"font-handwriting text-lg md:text-xl max-w-xl mx-auto text-white/70 mb-2"}
                   style={{ display: !isAnimationVisible ? 'block' : 'none' }}>
                    is a multi-agent intelligence platform for exploration and automation. Your environment aware planet computer for your
                </p>
                <span className={"text-sm tracking-wider text-[#7CFC00] block"}
                      style={{ display: !isAnimationVisible ? 'block' : 'none' }}>
