...
export async function POST(request: NextRequest) {
  try {
    const { email, identity, message } = await request.json();
    ...
    const mailOptions = {
      ...
      subject: `New QCX Interest Form Submission from ${email}`,
      html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
          <h2 style="color: #333;">New Interest Form Submission</h2>
          ...
          <hr/>
          <p style="font-size: 0.9em; color: #555;">This email was sent from the QCX website interest form.</p>
        </div>
      `,
    };
